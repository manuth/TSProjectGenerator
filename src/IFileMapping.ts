...
export interface IFileMapping<T extends Answers>
{
     ...
     */
    Source: string | string[] | ((answers: T) => string) | ((answers: T) => string[]) | ((answers: T) => Promise<string>) | ((answers: T) => Promise<string[]>);

    /**
     * The context to use for copying the file-entry.
     */
    Context?: ((answers: T) => any) | ((answers: T) => Promise<any>);

    /**
     * Gets or sets the destination to save the component to.
     */
    Destination: IComponentDestination | string | ((answers: T) => string) | ((answers: T) => Promise<string>);
}